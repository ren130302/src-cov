   ...
   extends RecipeBookContainer<IInventory>
 {
   ...
   private World world;
    public MillstoneContainer(int windowId, PlayerInventory playerInventory) {
     this(HiganContainerType.MILLSTONE, windowId, playerInventory, (IInventory)new Inventory(3));
   }
  public MillstoneContainer(int windowId, PlayerInventory playerInventory, MillstoneTileEntity inventory, IntArray progress) {
     this(HiganContainerType.MILLSTONE, windowId, playerInventory, (IInventory)inventory);
     this.tile = inventory;
   }
  public MillstoneContainer(ContainerType<?> type, int id, PlayerInventory playerInventory, IInventory inventory) {
     ...
     func_216959_a((IIntArray)this.tracker, 2);
        func_75146_a(new Slot(inventory, 0, 80, 9));
     func_75146_a(new NonInsertSlot(inventory, 1, 58, 57));
     func_75146_a(new NonInsertSlot(inventory, 2, 102, 57));
        int i;
     for (i = 0; i < 3; i++) {
       for (int j = 0; j < 9; j++) {
       ...
       func_75146_a(new Slot((IInventory)playerInventory, i, 8 + i * 18, 142));
     }
   }
  ...
  public int getGrindtime() {
   ...
   public ItemStack func_82846_b(PlayerEntity par1EntityPlayer, int par2) {
     ...
     Slot slot = this.field_75151_b.get(par2);
       ...
       itemstack = itemstack1.func_77946_l();
         ...
         } else if (par2 >= 30 && par2 < 39 && !func_75135_a(itemstack1, 3, 30, false)) {
           return ItemStack.field_190927_a;
   ...
   public List<RecipeBookCategories> getRecipeBookCategories() {
     return Lists.newArrayList((Object[])new RecipeBookCategories[] { HiganRecipeBookCategories.GRIND });
   }
 }
