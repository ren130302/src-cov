   ...
   extends Container
 {
   private IInventory inventory;
    public CollectorPressurePlateContainer(int windowId, PlayerInventory playerInventory) {
     this(HiganContainerType.COLLECTOR_PLATE, windowId, playerInventory, (IInventory)new Inventory(10));
   }
  public CollectorPressurePlateContainer(int windowId, PlayerInventory playerInventory, CollectorPressurePlateTileEntity inventory) {
     this(HiganContainerType.COLLECTOR_PLATE, windowId, playerInventory, (IInventory)inventory);
   }
  protected CollectorPressurePlateContainer(ContainerType<?> type, int id, PlayerInventory playerInventory, IInventory inventory) {
     ...
     for (int chestRow = 0; chestRow < 2; chestRow++) {
       for (int chestCol = 0; chestCol < 5; chestCol++) {
         func_75146_a(new Slot(inventory, chestCol + chestRow * 5, 44 + chestCol * 18, 33 + chestRow * 18));
       }
     }    int i;
     for (i = 0; i < 3; i++) {
       for (int j = 0; j < 9; j++) {
   ...
   public ItemStack func_184996_a(int slotId, int dragType, ClickType clickTypeIn, PlayerEntity player) {
     if (clickTypeIn == ClickType.QUICK_MOVE || clickTypeIn == ClickType.QUICK_CRAFT) {
     ...
     if (0 <= slotId && slotId < 10) {
       ...
       if (dragType == 0 &&        !stack.func_190926_b()) {
         ...
         func_75139_a(slotId).func_75215_d(copy);
       }           if (dragType == 1) {
         func_75139_a(slotId).func_75215_d(ItemStack.field_190927_a);
       }
       return ItemStack.field_190927_a;
